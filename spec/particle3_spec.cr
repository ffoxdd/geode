...
describe Particle(Vector3) do
  describe ".new" do
    it "returns a particle at the origin with radius 1" do
      Particle(Vector3).new.should eq(
        Particle(Vector3).new(center: Vector3.new({0.0, 0.0, 0.0}), radius: 1.0)
      )
    end
  end
  ...
  describe "#center" do
    it "exposes the center point" do
      ...
      particle = Particle(Vector3).new(center: center)
  ...
  describe "#magnitude" do
    it "returns the magnitude of the center point plus the radius" do
      particle = Particle(Vector3).new(center: Vector3.new({1.0, 2.0, 3.0}), radius: 1.0)
  ...
  describe "#aabb" do
    it "returns the aabb that covers the particle" do
      particle = Particle(Vector3).new(center: Vector3.new({1.0, 2.0, 3.0}), radius: 2.0)
  ...
  describe "#distance" do
    it "returns the distance to the other particle" do
      particle_1 = Particle(Vector3).new(
        center: Vector3.new({0.0, 0.0, 0.0}), radius: 1.0
      )

      particle_2 = Particle(Vector3).new(
